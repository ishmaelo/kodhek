...
def get_readings_for_display(conn,patient_id,start_date,end_date):
    cursor = conn.cursor()
    sql = "SELECT reading_date, reading, description FROM education WHERE patient_id = " + str(patient_id) + ""
    if start_date:
...
def get_readings_for_graph(conn,patient_id,start_date,end_date):
    cursor = conn.cursor()
    sql = "SELECT reading_date,scale FROM education WHERE patient_id = " + str(patient_id) + ""
    if start_date:
        sql += " AND reading_date BETWEEN DATE('" + str(start_date) + "') AND DATE('" + str(end_date) + "') ORDER BY reading_date ASC";
    else:
       sql += " ORDER BY reading_date DESC LIMIT 10 "
...
def check_next_appointment(st,conn,patient_id,widgets,components,utility):
    cursor = conn.cursor()
    sql = "SELECT reading_date FROM education WHERE patient_id = " + str(patient_id) + " ORDER BY reading_date DESC LIMIT 1"
    cursor.execute(sql)
...
def get_readings_for_score(conn,patient_id,start_date,end_date,st,utility,compute):
    cursor = conn.cursor()
    sql = "SELECT score FROM education WHERE patient_id = " + str(patient_id) + ""
    if start_date:
        sql += " AND reading_date BETWEEN DATE('" + str(start_date) + "') AND DATE('" + str(end_date) + "') ORDER BY reading_date ASC";
    else:
        sql += " ORDER BY reading_date DESC LIMIT 10 "
    ...
    st.session_state.education_score = average_score
...
def get_readings_for_concordance(conn,patient_id,start_date,end_date,st,pd,utility,compute):
    ...
    cursor = conn.cursor()
    sql = "SELECT reading_date,score FROM education WHERE patient_id = " + str(patient_id) + ""
    if start_date:
        sql += " AND reading_date BETWEEN DATE('" + str(start_date) + "') AND DATE('" + str(end_date) + "') ORDER BY reading_date ASC";
    else:
        sql += " ORDER BY reading_date DESC LIMIT 10 "
   ...
   #plot
    ...
    st.session_state.education_gradient = gradient
...
def get_all_score_description():
    scores = ["Undefined","Poor/ Very high distress","Fair/ High distress","Average/ Moderate distress","Good/ Mild distress","Excellent/ No distress","Education Rating Readings","Patient Education Performance","Education scores reading in scale"]
    return scores
...
def readings_on_table_display(readings,st,date_range):
    if len(readings)<1:
       ...
       return
    st.write("**PEducation " + date_range + "**")
    date, bgm, description = st.columns(3)
    with date:
       st.write("**Date**")
    with bgm:
       st.write("**Education Scores**")
    with description:
...
def initial_diagnosis_correlation_readings(conn, patient_id, diagnosis_date):
    cursor = conn.cursor()
    sql = "SELECT scale, score FROM education WHERE patient_id = " + str(patient_id) + " AND reading_date = '" + str (diagnosis_date) + "'";
    cursor.execute(sql)
...
def initial_diagnosis_correlation_readings_more(conn, patient_id, old_date_str,new_date_str, initial_readings, initial_readings_scores):
    cursor = conn.cursor()
    sql = "SELECT scale, score FROM education WHERE patient_id = " + str(patient_id) + " AND reading_date BETWEEN DATE('" + str(old_date_str) + "') AND DATE('" + str(new_date_str) + "')"
    cursor.execute(sql)
    rows = cursor.fetchall()
...
def save_to_db(conn,patient_id,reading_date,reading,scale,score,description,data_id):
    cursor=conn.cursor()
    if data_id:
        sql_update_query = """UPDATE education set reading_date = ?,reading=?, scale=?, score=?, description=? where id = ?"""
...
def delete_readings(conn,st,data_id):
    cursor = conn.cursor()
    sql_delete_query = "DELETE FROM education where id = " + str(data_id)
...
def get_readings_for_edit(st,conn,patient_id):
    cursor = conn.cursor()
    sql = "SELECT reading_date,reading, score, scale, description,id FROM education WHERE patient_id = " + str(patient_id) + " ORDER BY id ASC"
    cursor=conn.cursor()
    cursor.execute(sql)
    df = pd.DataFrame(cursor.fetchall(),columns=['Reading Date','Key Behavioral Measure','Score','Scale','Description','ID'])
    ...
    if st.button('Save Changes',key="bp_btn"):
        ...
        edited_rows = st.session_state["education_df"].get("edited_rows")
        ...
        for index,item in edited_items_list:
            ...
            test = df.loc[index, 'Key Behavioral Measure']
                    
            #check what has changed
            if 'Reading Date' in item:
                if reading_date != item['Reading Date']:
                    reading_date = item['Reading Date']
            if  'Key Behavioral Measure' in item:
                if test != item['Key Behavioral Measure']:
                    test = item['Key Behavioral Measure']
            save_readings(conn,st,0,test,reading_date,index+1,data_id)
        ##New Records
        added_rows = st.session_state["education_df"].get("added_rows")
        index = 0
        ...
        ##Delete Records
        deleted_rows = st.session_state["education_df"].get("deleted_rows")
        index = 0
