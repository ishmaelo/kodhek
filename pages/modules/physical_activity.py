...
def get_readings_for_graph(conn,patient_id,start_date,end_date):
    cursor = conn.cursor()
    sql = "SELECT reading_date,scale FROM physical_activity WHERE patient_id = " + str(patient_id) + ""
    if start_date:
        sql += " AND reading_date BETWEEN DATE('" + str(start_date) + "') AND DATE('" + str(end_date) + "') ORDER BY reading_date ASC";
    else:
       sql += " ORDER BY reading_date DESC LIMIT 10 "
...
def check_next_appointment(st,conn,patient_id,widgets,components,utility):
    cursor = conn.cursor()
    sql = "SELECT reading_date FROM physical_activity WHERE patient_id = " + str(patient_id) + " ORDER BY reading_date DESC LIMIT 1"
    cursor.execute(sql)
...
def get_readings_for_score(conn,patient_id,start_date,end_date,st,utility,compute):
    cursor = conn.cursor()
    sql = "SELECT score FROM physical_activity WHERE patient_id = " + str(patient_id) + ""
    if start_date:
        sql += " AND reading_date BETWEEN DATE('" + str(start_date) + "') AND DATE('" + str(end_date) + "') ORDER BY reading_date ASC";
    else:
        sql += " ORDER BY reading_date DESC LIMIT 10 "
    ...
    st.session_state.physical_activity_score = average_score
...
def get_readings_for_concordance(conn,patient_id,start_date,end_date,st,pd,utility,compute):
    ...
    cursor = conn.cursor()
    sql = "SELECT reading_date,score FROM physical_activity WHERE patient_id = " + str(patient_id) + ""
    if start_date:
        sql += " AND reading_date BETWEEN DATE('" + str(start_date) + "') AND DATE('" + str(end_date) + "') ORDER BY reading_date ASC";
    else:
        sql += " ORDER BY reading_date DESC LIMIT 10 "
   ...
   #plot
    ...
    st.session_state.physical_activity_gradient = gradient
...
def get_all_score_description():
    scores = ["Undefined","Very Poor","Poor","Fair","Good","Optimal","Physical Activity Readings","Patient Physical Activity Performance","Physical activity scores reading in scale"]
    return scores
...
def load_readings_with_chart(patient_id,st,conn,utility,pd,alt,datetime,start_date,end_date,date_range,widgets,components,compute):
    if not compute:
        st.markdown("""---""")
        st.subheader("10. Physical Activity")
...
def readings_on_table_display(readings,st,date_range):
    if len(readings)<1:
       ...
       return
    st.write("**Physical Activity " + date_range + "**")
    date, bgm, description = st.columns(3)
    with date:
       st.write("**Date**")
    with bgm:
       st.write("**MET min/week**")
    with description:
...
def initial_diagnosis_correlation_readings(conn, patient_id, diagnosis_date):
    cursor = conn.cursor()
    sql = "SELECT scale, score FROM physical_activity WHERE patient_id = " + str(patient_id) + " AND reading_date = '" + str (diagnosis_date) + "'";
    cursor.execute(sql)
...
def initial_diagnosis_correlation_readings_more(conn, patient_id, old_date_str,new_date_str, initial_readings, initial_readings_scores):
    cursor = conn.cursor()
    sql = "SELECT scale, score FROM physical_activity WHERE patient_id = " + str(patient_id) + " AND reading_date BETWEEN DATE('" + str(old_date_str) + "') AND DATE('" + str(new_date_str) + "')"
    cursor.execute(sql)
    rows = cursor.fetchall()
...
def save_to_db(conn,patient_id,reading_date,reading,total,percentage,scale,score,description,data_id):
    cursor=conn.cursor()
    if data_id:
        sql_update_query = """UPDATE physical_activity set reading_date = ?,reading=?, total=?, percentage=?,  scale=?, score=?, description=? where id = ?"""
...
def get_readings_for_edit(st,conn,patient_id):
    cursor = conn.cursor()
    sql = "SELECT reading_date,reading, total, percentage, score, scale, description,id FROM physical_activity WHERE patient_id = " + str(patient_id) + " ORDER BY id ASC"
    cursor=conn.cursor()
    cursor.execute(sql)
    df = pd.DataFrame(cursor.fetchall(),columns=['Reading Date','MET min/week','Total MET min/week','MET min/week %','Score','Scale','Description','ID'])
    df['Reading Date'] = pd.to_datetime(df['Reading Date'])
    edited_df = st.data_editor(
    df,
    key="physical_activity_df",
    num_rows="dynamic",
    disabled=['MET min/week %','Score','Scale','Description','ID'],
    column_config={
    ...
    st.write('In order to add or update an Physical Activity record, enter/alter Reading Date, the MET min/week, Total MET min/week. The system will automatically fill in the values for the other columns.')    
    if st.button('Save Changes',key="bp_btn"):
        ...
        for index,item in edited_items_list:
            ...
            total = df.loc[index, 'Total MET min/week']        
            ...
            if  'Total MET min/week' in item:
                if total != item['Total MET min/week']:
                    total = item['Total MET min/week']
