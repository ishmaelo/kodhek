...
def set_data_capture_form(conn,patient_id,st,widgets,components):   
    ##modal widgets##
    modal = widgets.create_modal_widget("Health System Scoring","health_system",50,600)
    open_modal = st.button("Health System Scoring","health_system")
    ...
    if modal.is_open():
        ...
        with modal.container():
            ...
            score = ''
            scale = ''
            reading_date = st.date_input("Reading date",format="YYYY-MM-DD",key="health_system-date")
            
            mmol = st.number_input("HBA1C %:",key="health_system-reading")
            if st.button('Submit reading',key="health_system-submit"):
