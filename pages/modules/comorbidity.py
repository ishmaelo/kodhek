...
def set_data_capture_form(conn,patient_id,st,widgets,components):   
    ##modal widgets##
    modal = widgets.create_modal_widget("Capture Comorbidity Score","comorbidity",50,600)
    open_modal = st.button("Capture Comorbidity Score","comorbidity")
    ...
    if modal.is_open():
        ...
        with modal.container():
            ...
            score = ''
            scale = ''
            reading_date = st.date_input("Reading date",format="YYYY-MM-DD",key="comorbidity-date")
            
            mmol = st.number_input("HBA1C %:",key="comorbidity-reading")
            if st.button('Submit reading',key="comorbidity-submit"):
