...
def set_data_capture_form(conn,patient_id,st,widgets,components):   
    ##modal widgets##
    modal = widgets.create_modal_widget("Capture Monofilament Test reading","monofilament",50,600)
    open_modal = st.button("Capture Monofilament Test reading","monofilament")
    ...
    if modal.is_open():
        ...
        with modal.container():
            ...
            score = ''
            scale = ''
            reading_date = st.date_input("Reading date",format="YYYY-MM-DD",key="monofilament-date")
            
            mmol = st.number_input("HBA1C %:",key="monofilament-reading")
            if st.button('Submit reading',key="monofilament-submit"):
